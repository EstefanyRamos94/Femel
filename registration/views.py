...
from .forms import UserCreationWithEmail, ProfileForm, PqrsForm       # Clases de formularios en forms.py
from django.views.generic.edit import CreateView, UpdateView            # Vista en clase para actualización de datos
...
@method_decorator(login_required, name='dispatch')
class PqrsCreate(UpdateView):
    form_class = PqrsForm
    success_url = reverse_lazy('pqrs')
    template_name = 'registration/profile_pqrs.html'

    # Mostrar el perfil a editar (logueado):
    def get_object(self):
        pqrs, created = Quejas.objects.get_or_create(usuario=self.request.user)
        return pqrs
